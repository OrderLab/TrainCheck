...
def get_functions(module, visited=None):
    ...
    for name, obj in inspect.getmembers(module):
        try:
            ...
            # must be a torch function or submodule
            if name == 'nn':
        ...
        try:
            ...
            for line in source.splitlines():
                ...
                for other_func_name in func_names:
                    if re.search(r'\b{}\b'.format(other_func_name.split('.')[-1]), line) and func_names[other_func_name] != func:
...
if specific_func_name in distances:
    print(f"Distance to C-level function for {specific_func_name}: {distances[specific_func_name]}")
else:
