...
def load_cora(path="./cora", device="cpu"):
    ...
    degree_mat = torch.sum(
        adj_mat, dim=1
    )  # Compute the sum of each row in the adjacency matrix (degree matrix)
    degree_mat = torch.sqrt(
        1 / degree_mat
    )  # Compute the reciprocal square root of the degrees
    degree_mat[degree_mat == float("inf")] = 0  # Handle division by zero cases
    degree_mat = torch.diag(
        degree_mat
    ).to_sparse()  # Convert the degree matrix to a sparse diagonal matrix
...
def train_iter(
    ...
    output = model(*input)
    loss = criterion(
        output[mask_train], target[mask_train]
    ...
    if epoch % print_every == 0:
        ...
        print(
            f"Epoch: {epoch:04d} ({(time.time() - start_t):.4f}s) loss_train: {loss_train:.4f} acc_train: {acc_train:.4f} loss_val: {loss_val:.4f} acc_val: {acc_val:.4f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--l2", type=float, default=5e-4, help="weight decay (default: 5e-4)"
    )
    parser.add_argument(
        "--dropout-p",
        type=float,
