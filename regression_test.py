...
def main():
    ...
    print(
        f"Source code instrumentation finished. Running the instrumented program with CUDA {'on' if torch.cuda.is_available() else 'off'}"
    )
    ...
    print("Finished running the instrumented program.")
    ...
    for loop_idx, (idx1, idx2) in enumerate(
        zip(instrumented_accuracy_indices, normal_accuracy_indices)
    ):
        instrumented_acc = instrumented_program_output[
            idx1 + PREFIX_LENGTH : idx1 + PREFIX_LENGTH + LOSS_SIG_FIGS
        ]
        normal_acc = normal_program_output[
            idx2 + PREFIX_LENGTH : idx2 + PREFIX_LENGTH + LOSS_SIG_FIGS
        ]
        print(instrumented_acc, normal_acc)
        if (
            abs(float(instrumented_acc) - float(normal_acc))
            > TEST_LOSS_MAX_INTERMEDIATE_ALLOWABLE_DIFF
        ):
            print(
                f"Training losses significantly different at epoch {loop_idx}, instrumented: {instrumented_acc}, normal: {normal_acc}"
            )
        else:
            print(
                f"Training losses not significantly different at epoch {loop_idx}, instrumented: {instrumented_acc}, normal: {normal_acc}"
            )
    ...
    if (
        abs(float(instrumented_acc) - float(normal_acc))
        > TEST_LOSS_FINAL_MAX_ALLOWABLE_DIFF
    ):
