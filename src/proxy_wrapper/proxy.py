...
def get_meta_vars(level = 5):
    ...
    for i in range(level):
        important_vars.update({key: frame_vars[key] for key in frame_vars 
...
def torch_serialize(obj):
    ...
    if isinstance(obj, torch.Tensor):
        new_value = dump_tensor(new_value)
        return new_value
    if isinstance(obj, torch.nn.Module):
        new_value = new_value.__class__.__name__ + '(nn.Module)'
        return new_value
    else:
...
class Proxy():
    ...
    def __init__(self, obj, logdir, log_level):
        ...
        if type(obj) is Proxy:
            self.logger_proxy.debug(f"Object '{obj.__class__.__name__}' is already a proxy")
            self._obj = obj._obj
            ...
            if type(obj) is torch.Tensor:
                if Proxy.tensor_frame_dict.get(tuple(frame_array)) is None:
                    ...
                    Proxy.tensor_frame_dict[tuple(frame_array)] = {tuple(obj.shape): self}
                else:
                    tensor_dict = Proxy.tensor_frame_dict.get(tuple(frame_array))
                    shape=tuple(obj.shape)
                    
                    if tensor_dict.get(shape) is None:
                        self.logger_proxy.info(f"Creating proxy for Tensor with shape '{shape}'")
                        
                        self.print_tensor(obj, logging.INFO)
                        self.__dict__['_obj'] = obj
                        tensor_dict[shape] = self
                    else:
                        self.logger_proxy.debug(f"Tensor with shape '{shape}' is already proxied")
                ...
                if Proxy.frame_dict.get(tuple(frame_array)) is None:
                    ...
                    if hasattr(obj, '__name__'):
                        self.logger_proxy.info(f"Creating proxy for object '{obj.__name__}'")
                        
                        self.jsondumper.dump_json(self.process_id, self.thread_id, get_meta_vars(), obj.__name__, {"old_value": None, "new_value": new_value})
                    else:
                        self.logger_proxy.info(f"Creating proxy for object with type '{obj.__class__.__name__}'")
                    
                        self.jsondumper.dump_json(self.process_id, self.thread_id, get_meta_vars(), obj.__class__.__name__, {"old_value": None, "new_value": new_value})
                    ...
                    if not type(obj) in [int, float, str, bool] and obj is not None:
                        self.logger_proxy.debug(f"Object '{obj.__class__.__name__}' is already proxied")
                    ...
                    self.jsondumper.dump_json(self.process_id, self.thread_id, get_meta_vars(), obj_name, {"old_value":old_value, "new_value": new_value})
    ...
    def __torch_function__(self, func, types, args=(), kwargs=None):
        self.logger_proxy.debug(f"Go to __torch_function__ for function '{func.__name__}'")
        if kwargs is None:
            kwargs = {}
    ...
    def __call__(self, *args, **kwargs):
        self.logger_proxy.debug(f"Go to __call__ for object '{self.__class__.__name__}'")
        args = tuple(arg._obj if (type(arg) is Proxy) else arg for arg in args)
        kwargs = {k: v._obj if (type(v) is Proxy) else v for k, v in kwargs.items()}
    ...
    def __setattr__(self, name, value):
        ...
        if name == '_obj':
            ...
            self.jsondumper.dump_json(self.process_id, self.thread_id, get_meta_vars(), attr_name, 
    ...
    def __add__(self, other):
        ...
        self.logger_proxy.debug(f"Calling __add__ for object '{self.__class__.__name__}'")
        other = other._obj if isinstance(other, Proxy) else other
        return self._obj + other
    
    def __or__(self, other):
        self.logger_proxy.debug(f"Calling __or__ for object '{self.__class__.__name__}'")
    ...
    def __ior__(self, other):
        self.logger_proxy.debug(f"Calling __ior__ for object '{self.__class__.__name__}'")
        if isinstance(other, bool):
    ...
    def __ror__(self, other):
        self.logger_proxy.debug(f"Calling __ror__ for object '{self.__class__.__name__}'")
        if isinstance(other, bool):
    ...
    def __radd__(self, other):
        self.logger_proxy.debug(f"Calling __radd__ for object '{self.__class__.__name__}'")
        # Unwrap other if it's a Proxy
        other = other._obj if isinstance(other, Proxy) else other
        return other + self._obj
    
    def __iadd__(self, other):
        self.logger_proxy.debug(f"Calling __iadd__ for object '{self.__class__.__name__}'")
        # Unwrap other if it's a Proxy
        other = other._obj if isinstance(other, Proxy) else other
        self._obj += other
        return self
    ...
    def __sub__(self, other):
        self.logger_proxy.debug(f"Calling __sub__ for object '{self.__class__.__name__}'")
        # Unwrap other if it's a Proxy
        other = other._obj if isinstance(other, Proxy) else other
        return self._obj - other
    
    def __mul__(self, other):
        self.logger_proxy.debug(f"Calling __mul__ for object '{self.__class__.__name__}'")
        # Unwrap other if it's a Proxy
        other = other._obj if isinstance(other, Proxy) else other
        return self._obj * other
    
    def __truediv__(self, other):
        self.logger_proxy.debug(f"Calling __truediv__ for object '{self.__class__.__name__}'")
        # Unwrap other if it's a Proxy
        other = other._obj if isinstance(other, Proxy) else other
        return self._obj / other
    
    def __floatdiv__(self, other):
        self.logger_proxy.debug(f"Calling __floatdiv__ for object '{self.__class__.__name__}'")
        # Unwrap other if it's a Proxy
        other = other._obj if isinstance(other, Proxy) else other
        return self._obj // other
    
    def __float__(self):
        self.logger_proxy.debug(f"Calling __float__ for object '{self.__class__.__name__}'")
        return float(self._obj)
    
    def __int__(self):
        self.logger_proxy.debug(f"Calling __int__ for object '{self.__class__.__name__}'")
        return int(self._obj)
    
    def __str__(self):
        self.logger_proxy.debug(f"Calling __str__ for object '{self.__class__.__name__}'")
        return str(self._obj)
    
    def __bool__(self):
        self.logger_proxy.debug(f"Calling __bool__ for object '{self.__class__.__name__}'")
        return bool(self._obj)
    
    def __repr__(self):
        self.logger_proxy.debug(f"Calling __repr__ for object '{self.__class__.__name__}'")
        return repr(self._obj)
    
    def __len__(self):
        self.logger_proxy.debug(f"Calling __len__ for object '{self.__class__.__name__}'")
