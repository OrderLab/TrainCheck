...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def __init__(self, filenames, root=None, logger=None):
                    ...
                    for from_node, to_node in import_mapping.items():
                        if (
                            ...
                            if to_node not in self.defines_edges:
                                # TODO: some nodes are not in the self.defines_edges:
                                # E.g., ..nn.module.torch.ao.quantization.utils, torch._C, ..export.graph_signature.InputKind, ...
                                continue
    ...
    def visit_FunctionDef(self, node):
        ...
        if inner_ns not in self.scopes:
            ...
            for key in self.scopes.keys():
                if key.startswith(split_inner_ns[0]) and key.endswith(
                    split_inner_ns[1]
                ):
                    inner_ns = key
                    # missing_ns is the namespace that is missing
                    missing_ns = inner_ns.replace(split_inner_ns[0], "").replace(
                        split_inner_ns[1], ""
                    )
        ...
        ), "Name stack mismatch, expected %s, got %s" % (node.name, self.name_stack[-1])
        self.name_stack.pop()
    ...
    def get_node_of_current_namespace(self):
        ...
        if hasattr(self, "class_stack") and len(self.class_stack):
            class_namespace = self.class_stack[
                -1
            ].namespace  # + "." + self.class_stack[-1].name
            ...
            if class_namespace.startswith(namespace):
                if (
                    self.class_stack[-1].name != name
                ):  # representing a class method here
                    ...
                    self.context_stack.append(
                        "ClassDef %s" % (self.class_stack[-1].name)
                    )
    ...
    def find_top_level_nodes(self):
        self.all_nodes = {
            item
            for sublist in self.nodes.values()
            for item in sublist
            if item.flavor in self.filter_flavor
        }
    ...
    def build_graph(self):
        ...
        for from_node, to_nodes in self.uses_edges.items():
            for to_node in to_nodes:
                if (
                    to_node.flavor in self.filter_flavor
                    and from_node.flavor in self.filter_flavor
                ):
    ...
    def dump_levels(
        self,
        output_path="debug_level.log",
        filter_level=None,
        filter_func_set=None,
        show_hidden=True,
    ):
        ...
        with open(output_path, "w") as f:
            for node, level in self.level.items():
                if (
                    node.flavor != Flavor.ATTRIBUTE
                    and (filter_level is None or level <= filter_level)
                    and (filter_func_set is None or filter_func(node))
                ):
                    if not show_hidden:
