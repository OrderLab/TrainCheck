...
class TraceCache:
    ...
    def load_log_data(self, log_data):
        for entry in log_data:
            ...
            elif entry['type'] in [TraceLineType.FUNC_CALL_PRE, TraceLineType.FUNC_CALL_POST, TraceLineType.FUNC_CALL_POST_EXCEPTION]:
    ...
    def get_start_time(self, process_id=None, thread_id=None) -> float:
        ...
        if process_id is not None:
            start_times = [min(event["time"] for event in times) for times in self.events[process_id].values()]
    ...
    def get_end_time(self, process_id=None, thread_id=None) -> float:
        ...
        if process_id is not None:
            end_times = [max(event["time"] for event in times) for times in self.events[process_id].values()]
...
def read_trace_file_dict(
    ...
    return TraceCache(file_path, truncate_incomplete_func_calls)