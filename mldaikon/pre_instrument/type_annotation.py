...
def type_annotation_from_trace(trace_path: str):
    ...
    combined_results: DefaultDict[str, DefaultDict[str, Set[str]]] = defaultdict(
    ...
    with open(trace_path, "r", encoding="utf-8") as file:
        for line in file:
            ...
            extracted_data = {
                key: item[key] for key in elements_to_extract if key in item
            }
            ...
            if "return_type" in extracted_data:
                combined_results[extracted_data["function"]]["return_type"].add(
                    extracted_data["return_type"]
                )

    final_result = {
        func: {k: list(v) for k, v in args.items()}
        for func, args in combined_results.items()
    }
...
def type_annotation_from_traces(
    trace_paths: list[str], output_path: str = "output.json"
):
    ...
    print(
        f"Extracted {len(func_to_annotations)} functions from {len(trace_paths)} trace files"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process trace files for type annotation."
    )
    parser.add_argument(
        "-f", "--files", nargs="+", required=True, help="List of trace files to process"
    )
